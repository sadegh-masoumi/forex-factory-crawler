...
def fetch_forex_factory_economic_calendar():
    ...
    if response.status_code == 200:
        ...
        for row in event_rows:
            try:
                event_time = row.find("td", class_="calendar__time").text.strip()
                event_currency = row.find(
                    "td", class_="calendar__currency"
                ).text.strip()
                event_impact = row.find("td", class_="calendar__impact").span["title"]
                event_description = row.find(
                    "td", class_="calendar__event"
                ).text.strip()
                event_actual = row.find("td", class_="calendar__actual").text.strip()
                event_forecast = row.find(
                    "td", class_="calendar__forecast"
                ).text.strip()
                event_previous = row.find(
                    "td", class_="calendar__previous"
                ).text.strip()
